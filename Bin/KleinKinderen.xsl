...
// <file>$Workfile: KleinKinderen.xsl $</file><date>$Date: 21-10-13 11:14 $</date>
    ...
    <xsl:choose>
      <xsl:when test="count($kkind/PERSOON) = 0">
        <xsl:for-each select="//PERSOON[@link=$HuidigPersoon]/PERSONAL/NAME">
          <h2 align="center">
            <xsl:value-of select="$Txt_Kleinkinderentitelgeen"/>
      ...
      <xsl:otherwise>  
        <xsl:for-each select="//PERSOON[@link=$HuidigPersoon]/PERSONAL/NAME">
          <h2 align="center">
            <xsl:value-of select="$Txt_Overzicht"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_vande"/><xsl:text> </xsl:text><xsl:value-of select="$cntr"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_kleinkinderen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text>
            <xsl:value-of select="@first"/>
            <xsl:if test="string-length(@nick) > 0"><xsl:text> </xsl:text>(<xsl:value-of select="@nick"/>)</xsl:if>
            <xsl:if test="string-length(@pref) > 0"><xsl:text> </xsl:text><xsl:value-of select="@pref"/></xsl:if>
            <xsl:text> </xsl:text><xsl:value-of select="@surn"/>
    ...
    <xsl:for-each select="RELATIONS/RELA/CHILDREN/CHIL">
      ...
      <xsl:for-each select="//PERSONEN/PERSOON[@link=$kind]">
        <xsl:for-each select="RELATIONS/RELA/CHILDREN/CHIL">
          ...
          <xsl:copy-of select="//PERSONEN/PERSOON[@link=$kleinkind]"/>
        </xsl:for-each>
      </xsl:for-each>
    </xsl:for-each>
  </xsl:variable>
