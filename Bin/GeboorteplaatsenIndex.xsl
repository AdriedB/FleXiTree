...
// <file>$Workfile: GeboorteplaatsenIndex.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
*********************************************************************************** -->
<xsl:key name="items-index" match="//PERSONEN/PERSOON" use="EVENTS/BIRTH/@place"/>
...
Template :   Onderstaande template beeld een tabel af.
             ...
             de kolom aantal.
  <xsl:for-each select="PERSOON[generate-id() = generate-id(key('items-index',EVENTS/BIRTH/@place)[1]) and PERSONAL/NAME/@surn='Blanken']">
Parameters : Geen
*********************************************************************************** -->
<xsl:template name="items-lijst">
  ...
  <xsl:for-each select="PERSOON[EVENTS/BIRTH/@place != '' and generate-id() = generate-id(key('items-index',EVENTS/BIRTH/@place)[1])]">
    <xsl:sort select="EVENTS/BIRTH/@place" order="ascending" data-type="text"/>
    <xsl:sort select="EVENTS/BIRTH/@date" order="ascending" data-type="text"/>
    <xsl:variable name="selectie" select="key('items-index',EVENTS/BIRTH/@place)"/>
      ...
      <td class="tbl-td-noWrap">
        <xsl:variable name="placeNoQuote">
          <xsl:call-template name="replace-substring">
            <xsl:with-param name="original"><xsl:value-of select="EVENTS/BIRTH/@place"/></xsl:with-param>
        ...
        <xsl:element name="a">
           <xsl:attribute name="href">javascript:KiesXSL(23,<xsl:value-of select="$HuidigPersoon"/>,'<xsl:value-of select="$placeNoQuote"/>','')</xsl:attribute>
           <xsl:value-of select="EVENTS/BIRTH/@place"/>
        </xsl:element>
      </td>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[generate-id() = generate-id(key('items-index',EVENTS/BIRTH/@place)[1])])"/>
   	</xsl:variable>
  ...
  <div id="Layer1" style="z-index:1" align="center">
    	...
    	<table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
          <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">6</xsl:with-param>
              <xsl:with-param name="align">center</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
              <xsl:with-param name="name"><xsl:value-of select="$Txt_Geboorteplaats"/></xsl:with-param>
              <xsl:with-param name="sort">5</xsl:with-param>
