...
// <file>$Workfile: AchternamenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_achternaam"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
...
<xsl:template name="AchternamenlijstWeergave">
       ...
       <tr>
         ...
         <xsl:call-template name="addColumn">
          ...
          <xsl:for-each select="PERSOON[PERSONAL/NAME/@surn=$LijstWoord]">
            ...
            <xsl:call-template name="AchternamenlijstWeergave">
              <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
