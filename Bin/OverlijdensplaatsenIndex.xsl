...
// <file>$Workfile: OverlijdensplaatsenIndex.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
*********************************************************************************** -->
<xsl:key name="items-index" match="//PERSONEN/PERSOON" use="EVENTS/DEATH/@place"/>
...
Template :   Onderstaande template beeld een tabel af.
             ...
             de kolom aantal.
  <xsl:for-each select="PERSOON[generate-id() = generate-id(key('items-index',EVENTS/DEATH/@place)[1]) and PERSONAL/NAME/@surn='Blanken']">
Parameters : Geen
*********************************************************************************** -->
<xsl:template name="items-lijst">
  ...
  <xsl:for-each select="PERSOON[EVENTS/DEATH/@place != '' and generate-id() = generate-id(key('items-index',EVENTS/DEATH/@place)[1])]">
    <xsl:sort select="EVENTS/DEATH/@place" order="ascending" data-type="text"/>
    <xsl:sort select="EVENTS/DEATH/@date" order="ascending" data-type="text"/>
    <xsl:variable name="selectie" select="key('items-index',EVENTS/DEATH/@place)"/>
      ...
      <td class="tbl-td-noWrap">
        <xsl:variable name="placeNoQuote">
          <xsl:call-template name="replace-substring">
            <xsl:with-param name="original"><xsl:value-of select="EVENTS/DEATH/@place"/></xsl:with-param>
        ...
        <xsl:element name="a">
           <xsl:attribute name="href">javascript:KiesXSL(63,<xsl:value-of select="$HuidigPersoon"/>,'<xsl:value-of select="$placeNoQuote"/>','')</xsl:attribute>
           <xsl:value-of select="EVENTS/DEATH/@place"/>
        </xsl:element>
      </td>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[generate-id() = generate-id(key('items-index',EVENTS/DEATH/@place)[1])])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
   	  ...
   	  <xsl:with-param name="overzichtstype_enkelvoud">
   	    <xsl:value-of select="$Txt_overlijdensplaats"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_meervoud">
   	    <xsl:value-of select="$Txt_overlijdensplaatsen"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_met">
   	  </xsl:with-param>
   	  <xsl:with-param name="sleutel">
        <xsl:choose>
          <xsl:when test="$parSortKey=10">
    	...
    	<table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
          <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">10</xsl:with-param>
              <xsl:with-param name="align">center</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
              <xsl:with-param name="name"><xsl:value-of select="$Txt_Overlijdensplaats"/></xsl:with-param>
              <xsl:with-param name="sort">9</xsl:with-param>
