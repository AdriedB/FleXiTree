...
// <file>$Workfile: WoonplaatsenIndex.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
*********************************************************************************** -->
<xsl:key name="items-index" match="//PERSONEN/PERSOON" use="FACTS/FACT/ADDR/@city"/>
...
<xsl:template name="items-lijst">
  ...
  <xsl:for-each select="PERSOON[FACTS/FACT/ADDR/@city != '' and generate-id() = generate-id(key('items-index',FACTS/FACT/ADDR/@city)[1])]">
    <xsl:sort select="FACTS/FACT/ADDR/@city" order="ascending" data-type="text"/>
    <xsl:sort select="PERSONAL/NAME/@first" order="ascending" data-type="text"/>
    <xsl:variable name="selectie" select="key('items-index',FACTS/FACT/ADDR/@city)"/>
      ...
      <td class="tbl-td-noWrap">
        <xsl:variable name="placeNoQuote">
          <xsl:call-template name="replace-substring">
            <xsl:with-param name="original"><xsl:value-of select="FACTS/FACT/ADDR/@city"/></xsl:with-param>
        ...
        <xsl:element name="a">
           <xsl:attribute name="href">javascript:KiesXSL(67,<xsl:value-of select="$HuidigPersoon"/>,'<xsl:value-of select="$placeNoQuote"/>','')</xsl:attribute>
           <xsl:value-of select="FACTS/FACT/ADDR/@city"/>
        </xsl:element>
      </td>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[generate-id() = generate-id(key('items-index',FACTS/FACT/ADDR/@city)[1])])"/>
   	</xsl:variable>
    	...
    	<table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
          <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">23</xsl:with-param>
              <xsl:with-param name="align">center</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
              <xsl:with-param name="name"><xsl:value-of select="$Txt_Woonplaats"/></xsl:with-param>
              <xsl:with-param name="sort">24</xsl:with-param>
