...
// <file>$Workfile: OverlijdensplaatsenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[EVENTS/DEATH/@place=$LijstWoord])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_overlijdensplaats"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="sleutel">
        <xsl:choose>
          <xsl:when test="$parSortKey=14">
...
<xsl:template name="OverlijdensplaatsenlijstWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
         <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">13</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
          ...
          <xsl:for-each select="PERSOON[EVENTS/DEATH/@place=$LijstWoord]">
       	    <xsl:sort order="ascending" select="sortkey1"/>
            <xsl:sort order="ascending" select="sortkey2"/>
            <xsl:sort order="ascending" select="sortkey3"/>
            <xsl:sort order="ascending" select="sortkey4"/>
            <xsl:call-template name="OverlijdensplaatsenlijstWeergave">
              <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
