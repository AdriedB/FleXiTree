...
// <file>$Workfile: Sier5G.xsl $</file><date>$Date: 24-02-13 13:44 $</date>
  ...
  <xsl:for-each select="PERSOON[@link=$HuidigPersoon]">
    ...
    <div align="center">
      <xsl:if test="$Printing = 1">
        <div align="center">
          <a href="javascript:KiesXSL(50,{$HuidigPersoon},'','');"
             onMouseOver = "window.status='{$Txt_terug}'; return true;"
             onMouseOut  = "window.status='';"><h3><xsl:value-of select="$Txt_Sierstamboom"/> (5 <xsl:value-of select="$Txt_generaties"/>) <xsl:value-of select="$Txt_van"/></h3></a>
        </div>
      </xsl:if>
      <xsl:if test="$Printing = 0">
        <h3><xsl:value-of select="$Txt_Sierstamboom"/> (5 <xsl:value-of select="$Txt_generaties"/>) <xsl:value-of select="$Txt_van"/></h3>
      </xsl:if>
      <h2>
      ...
      <xsl:if test="$LijstWoord = 'HQ'">
        <embed name="SVGEmbedIDAVJTVB" src="Svg/Sier5GHQ.svg.gz" width="900" height="625" pluginspage="http://www.adobe.com/svg/viewer/install/" type="image/svg+xml"></embed>
      </xsl:if>
      <xsl:if test="$LijstWoord = 'LQ'">
        <embed name="SVGEmbedIDAVJTVB" src="Svg/Sier5GLQ.svg.gz" width="900" height="625" pluginspage="http://www.adobe.com/svg/viewer/install/" type="image/svg+xml"></embed>
      </xsl:if>
    </div>
    ...
    <xsl:call-template name="SvgNavigatie">
        ...
        <xsl:with-param name="WelkeSvg">50</xsl:with-param>
