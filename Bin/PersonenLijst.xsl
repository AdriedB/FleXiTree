...
// <file>$Workfile: PersonenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam)))) and @link!=0])"/>
   	</xsl:variable>
...
<xsl:template name="PersonenlijstWeergave">
...
<xsl:template name="KolomWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
      ...
      <xsl:for-each select="PERSOON[($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam)))) and @link!=0]">
        <xsl:sort select="concat(PERSONAL/NAME/@nick,substring(PERSONAL/NAME/@first,1000*string-length(PERSONAL/NAME/@nick)))" order="ascending" data-type="text"/>
        ...
        <xsl:sort select="PERSONAL/NAME/@pref" order="ascending" data-type="text"/>
      	...
      	<xsl:call-template name="PersonenlijstWeergave">
            <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
