...
// <file>$Workfile: Levend.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[not(EVENTS/DEATH) and @link != 0 and ($Key2='0' or PERSONAL/@sex=$Key2) and ($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam))))])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:if test="$Key2='M'">
  		  <xsl:value-of select="$Txt_levendeman"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='V'">
  		  <xsl:value-of select="$Txt_levendevrouw"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='?'">
  		  <xsl:value-of select="$Txt_levendepersoon"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_onbekend"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_geslacht"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='0'">
  		  <xsl:value-of select="$Txt_levendepersoon"/>
  	    </xsl:if>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_meervoud">
        <xsl:if test="$Key2='M'">
  		  <xsl:value-of select="$Txt_levendemannen"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='V'">
  		  <xsl:value-of select="$Txt_levendevrouwen"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='?'">
  		  <xsl:value-of select="$Txt_levendepersonen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_onbekend"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_geslacht"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='0'">
  		  <xsl:value-of select="$Txt_levendepersonen"/>
  	    </xsl:if>
   	  </xsl:with-param>
...
<xsl:template name="LevendWeergave">
...
<xsl:template name="KolomWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
     <tr>
      ...
      <xsl:for-each select="PERSOON[not(EVENTS/DEATH) and @link != 0 and ($Key2='0' or PERSONAL/@sex=$Key2) and ($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam))))]">
    	...
    	<xsl:call-template name="LevendWeergave">
          <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
