...
// <file>$Workfile: KinBerekeningRelaties.xsl $</file><date>$Date: 2-12-13 16:06 $</date>
...
var max_top_coord;
var max_top_coord_tot=0;
var nr_block=new Array();
var nr_in_line=new Array();
var nr_matches=0;
var nr_match_relations=0;
var nr_pers=0;
...
var offset_x=150;
var offset_y_init=300;
var offset_y=offset_y_init;
var width_box=300;
var new_offset_y=offset_y_init;
...
function calc_block_top()
{
	...
	max_top_coord_tot=Math.max(max_top_coord_tot,max_top_coord);
	new_offset_y=offset_y_init + max_top_coord_tot;
	nr_match_relations++;
	}
	return "";
...
function write_offset_y()
{
		return offset_y;
}

function write_nr_match_relations()
{
	return nr_match_relations;
}
  ...
  <xsl:call-template name="ShowMenu"/>
	...
	<xsl:for-each select="PERSOON">
	  ...
	  <xsl:for-each select="RELATIONS/RELA">
	    ...
	    <xsl:if test="$link1 &lt; $link2">
	
<!-- rekenwerk in JS -->	
   	<xsl:value-of select="user:initialize()"/> 
   	<xsl:value-of select="user:read_pers(1,string($link1))"/> 
   	<xsl:value-of select="user:read_pers(2,string($link2))"/>
...
<!-- Schrijf uitvoer -->
    ...
    <xsl:if test="user:write_nr_match_relations()=1 and $NrMatches !=0">
      <h3 align="center">
         <xsl:value-of select="$Txt_Verwantschappen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_tussen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_personen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_relatie"/>
      </h3>
    </xsl:if>
	<xsl:if test="$NrMatches != '0'">
	  <xsl:call-template name="TitelTekst">
	    <xsl:with-param name="link1"><xsl:value-of select="$link1"/></xsl:with-param>
	    <xsl:with-param name="link2"><xsl:value-of select="$link2"/></xsl:with-param>
	    <xsl:with-param name="NrMatches"><xsl:value-of select="$NrMatches"/></xsl:with-param>
	  </xsl:call-template>
...
<!-- (Bijna) lege tabel boven -->	
    ...
    <xsl:if test="user:write_nr_match_relations()=0">
      <h3>
        <xsl:value-of select="$Txt_geen_relatie_verwantschap"/>
      </h3>
    </xsl:if>
...
<xsl:template name="TitelTekst">
  <xsl:param name="link1"/>
  <xsl:param name="link2"/>
  <xsl:param name="NrMatches"/>
  
    	<xsl:element name="table">
        	<xsl:attribute name="style">
            	position: absolute; border: 0px solid blue; border-spacing: 0; cell-padding: 0; cell-spacing: 0;
               	height:<xsl:value-of select="user:write_blok_hoogte()"/>px;
               	top:<xsl:value-of select="user:write_offset_y()-100"/>px;
               	left: 0px;
            </xsl:attribute>
			<tr>
   				<td align="left" valign="top" class="tbl-tekst">
   					<xsl:text> </xsl:text><xsl:value-of select="$Txt_Verwantschap"/><xsl:text> </xsl:text>
    <xsl:for-each select="//PERSONEN/PERSOON[@link=$link1]">
	...
	<xsl:call-template name="ShowPersoon">
          ...
          <xsl:with-param name="src"><xsl:value-of select="$link1"/></xsl:with-param>
          <xsl:with-param name="priv"><xsl:value-of select="../@priv"/></xsl:with-param>
        </xsl:call-template>
    </xsl:for-each>
  </xsl:for-each>
  <xsl:text> </xsl:text><xsl:value-of select="$Txt_en"/><xsl:text> </xsl:text>
    <xsl:for-each select="//PERSONEN/PERSOON[@link=$link2]">
    <xsl:for-each select="PERSONAL">
	<xsl:call-template name="ShowPersoon">
          <xsl:with-param name="NAMEFIRST"><xsl:value-of select="NAME/@first"/></xsl:with-param>
          <xsl:with-param name="NAMENICK"><xsl:value-of select="NAME/@nick"/></xsl:with-param>
          <xsl:with-param name="NAMEPREF"><xsl:value-of select="NAME/@pref"/></xsl:with-param>
          <xsl:with-param name="SURN"><xsl:value-of select="NAME/@surn"/></xsl:with-param>
          <xsl:with-param name="src"><xsl:value-of select="$link2"/></xsl:with-param>
          <xsl:with-param name="priv"><xsl:value-of select="../@priv"/></xsl:with-param>
        </xsl:call-template>
    </xsl:for-each>
  </xsl:for-each>
