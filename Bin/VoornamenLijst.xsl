...
// <file>$Workfile: VoornamenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[PERSONAL/NAME/@nick=$LijstWoord])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_roepnaam"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
...
<xsl:template name="VoornamenlijstWeergave">
    ...
    <xsl:if test="$lb &lt; position() and position() &lt; $ub">
        <tr>
          ...
          <!-- controlemogelijkheid op sorteersleutel
          <td>
            Key=<xsl:value-of select="concat(PERSONAL/NAME/@nick,substring(PERSONAL/NAME/@first,1000,string-length(PERSONAL/NAME/@nick)))"/>
          </td>
          -->
          <td align="right">
            <xsl:choose>
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
       <tr>
         ...
         <xsl:call-template name="addColumn">
          ...
          <xsl:for-each select="PERSOON[PERSONAL/NAME/@nick=$LijstWoord]">
            ...
            <xsl:call-template name="VoornamenlijstWeergave">
              <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
