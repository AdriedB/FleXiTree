...
// <file>$Workfile: Waaier-8G.xsl $</file><date>$Date: 24-02-13 13:44 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:for-each select="PERSOON[@link=$HuidigPersoon]">
    ...
    <div align="center">
      <xsl:if test="$Printing = 1">
        <div align="center">
          <a href="javascript:KiesXSL(58,{$HuidigPersoon},'','');"
             ...
             onMouseOut  = "window.status='';"><h3><xsl:value-of select="$Txt_Waaier"/> (8 <xsl:value-of select="$Txt_generaties"/>) <xsl:value-of select="$Txt_van"/></h3></a>
        </div>
      </xsl:if>
      <xsl:if test="$Printing = 0">
        <h3><xsl:value-of select="$Txt_Waaier"/> (8 <xsl:value-of select="$Txt_generaties"/>) <xsl:value-of select="$Txt_van"/></h3>
      </xsl:if>
      <h2>
      ...
      <embed name="SVGEmbedIDAVJTVB" src="Svg/Waaier8G.svg.gz" width="787" height="540" pluginspage="http://www.adobe.com/svg/viewer/install/" type="image/svg+xml"></embed>
    </div>
    ...
    <xsl:call-template name="SvgNavigatie">
        ...
        <xsl:with-param name="WelkeSvg">58</xsl:with-param>
