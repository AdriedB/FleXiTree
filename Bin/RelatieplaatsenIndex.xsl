...
// <file>$Workfile: RelatieplaatsenIndex.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
*********************************************************************************** -->
<xsl:key name="items-index" match="//PERSONEN/PERSOON/RELATIONS/RELA" use="BEGIN/@place"/>
...
<xsl:template name="items-lijst">
  ...
  <xsl:for-each select="PERSOON/RELATIONS/RELA[BEGIN/@place != '' and generate-id() = generate-id(key('items-index',BEGIN/@place)[1])]">
    <xsl:sort select="BEGIN/@place" order="ascending" data-type="text"/>
    <xsl:sort select="BEGIN/@date" order="ascending" data-type="text"/>
    <xsl:variable name="selectie" select="key('items-index',BEGIN/@place)"/>
    <xsl:if test="$lb &lt; position() and position() &lt; $ub">
    <tr align="left">
      <td align="center">
        <xsl:value-of select="ceiling(count($selectie) div 2)"/>
      </td>
      <td class="tbl-td-noWrap">
        <xsl:variable name="placeNoQuote">
          <xsl:call-template name="replace-substring">
            <xsl:with-param name="original"><xsl:value-of select="BEGIN/@place"/></xsl:with-param>
        ...
        <xsl:element name="a">
           <xsl:attribute name="href">javascript:KiesXSL(97,<xsl:value-of select="$HuidigPersoon"/>,'<xsl:value-of select="$placeNoQuote"/>','')</xsl:attribute>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON/RELATIONS/RELA[generate-id() = generate-id(key('items-index',BEGIN/@place)[1])])"/>
   	</xsl:variable>
  ...
  <div id="Layer1" style="z-index:1" align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
   	  ...
   	  <xsl:with-param name="overzichtstype_enkelvoud">
   	    <xsl:value-of select="$Txt_relatieplaats"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_meervoud">
   	    <xsl:value-of select="$Txt_relatieplaatsen"/>
   	  </xsl:with-param>
    	...
    	<table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
          <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">42</xsl:with-param>
              <xsl:with-param name="align">center</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
              <xsl:with-param name="name"><xsl:value-of select="$Txt_Relatieplaats"/></xsl:with-param>
              <xsl:with-param name="sort">41</xsl:with-param>
