...
// <file>$Workfile: AchternamenLetterIndex.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template name="items-lijst">
  ...
  <xsl:for-each select="PERSOON[substring(PERSONAL/NAME/@surn,1,1)=$LijstWoord and generate-id() = generate-id(key('items-index',PERSONAL/NAME/@surn)[1])]">
    ...
    <tr align="left">
      ...
      <td>
        ...
        <xsl:element name="a">
           <xsl:attribute name="href">javascript:KiesXSL(21,<xsl:value-of select="$HuidigPersoon"/>,'<xsl:value-of select="$surnNoQuote"/>','')</xsl:attribute>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[substring(PERSONAL/NAME/@surn,1,1)=$LijstWoord and generate-id() = generate-id(key('items-index',PERSONAL/NAME/@surn)[1])])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
   	  ...
   	  <xsl:with-param name="overzichtstype_meervoud">
   	    <xsl:value-of select="$Txt_achternamen"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_met">
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_meteersteletter"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="sleutel">
        <xsl:choose>
          <xsl:when test="$parSortKey=20">
