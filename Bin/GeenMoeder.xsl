...
// <file>$Workfile: GeenMoeder.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[not(MOTHER) and PERSONAL/NAME/@surn=$refAchternaam])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_zonder_moeder"/>
...
<xsl:template name="GeenMoederWeergave">
...
<xsl:template name="KolomWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
       <tr>
        ...
        <xsl:for-each select="PERSOON[not(MOTHER) and PERSONAL/NAME/@surn=$refAchternaam]">
          ...
          <xsl:call-template name="GeenMoederWeergave">
            <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
