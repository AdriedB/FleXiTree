...
// <file>$Workfile: DoopplaatsenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[EVENTS/BAPT/@place=$LijstWoord])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_doopplaats"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
   	  <xsl:with-param name="sleutel">
        <xsl:choose>
          <xsl:when test="$parSortKey=12">
...
<xsl:template name="DoopplaatsenlijstWeergave">
    ...
    <xsl:if test="$lb &lt; position() and position() &lt; $ub">
        <tr align="left">
          ...
          <td align="right">
            <xsl:choose>
              <xsl:when test="EVENTS/BAPT/@date">
      ...
      <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
         <tr>
           <xsl:call-template name="addColumn">
              ...
              <xsl:with-param name="sort">11</xsl:with-param>
           </xsl:call-template>
           <xsl:call-template name="addColumn">
              <xsl:with-param name="name"><xsl:value-of select="$Txt_Geboortedoopdatum"/></xsl:with-param>
              <xsl:with-param name="sort">12</xsl:with-param>
              <xsl:with-param name="align">right</xsl:with-param>
           </xsl:call-template>
          </tr>
          <xsl:for-each select="PERSOON[EVENTS/BAPT/@place=$LijstWoord]">
            ...
            <xsl:call-template name="DoopplaatsenlijstWeergave">
              <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
