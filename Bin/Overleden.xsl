...
// <file>$Workfile: Overleden.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[EVENTS/DEATH and ($Key2='0' or PERSONAL/@sex=$Key2) and ($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam))))])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:if test="$Key2='M'">
  		  <xsl:value-of select="$Txt_overledenman"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='V'">
  		  <xsl:value-of select="$Txt_overledenvrouw"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='?'">
  		  <xsl:value-of select="$Txt_overledenpersoon"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_onbekend"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_geslacht"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='0'">
  		  <xsl:value-of select="$Txt_overledenpersoon"/>
  	    </xsl:if>
   	  </xsl:with-param>
   	  <xsl:with-param name="overzichtstype_meervoud">
        <xsl:if test="$Key2='M'">
  		  <xsl:value-of select="$Txt_overledenmannen"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='V'">
  		  <xsl:value-of select="$Txt_overledenvrouwen"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='?'">
  		  <xsl:value-of select="$Txt_overledenpersonen"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_onbekend"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_geslacht"/>
  	    </xsl:if>
  	    <xsl:if test="$Key2='0'">
  		  <xsl:value-of select="$Txt_overledenpersonen"/>
  	    </xsl:if>
   	  </xsl:with-param>
...
<xsl:template name="OverledenWeergave">
    ...
    <xsl:if test="$lb &lt; $pos and $pos &lt; $ub">
        <tr>
          ...
          <td align="right">
            <xsl:choose>
              <xsl:when test="EVENTS/DEATH/@date">
                <xsl:value-of select="EVENTS/DEATH/@onz"/><xsl:value-of select="EVENTS/DEATH/@date"/><xsl:text>&#160;</xsl:text>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="EVENTS/BURI/@onz"/><xsl:value-of select="EVENTS/BURI/@date"/><xsl:text>&#160;</xsl:text>
              </xsl:otherwise>
            </xsl:choose>
          </td>
          <td align="right">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
     <tr>
       ...
       <xsl:call-template name="addColumn">
          ...
          <xsl:with-param name="sort">14</xsl:with-param>
          <xsl:with-param name="align">right</xsl:with-param>
       </xsl:call-template>
       <xsl:call-template name="addColumn">
      ...
      <xsl:for-each select="PERSOON[EVENTS/DEATH and ($Key2='0' or PERSONAL/@sex=$Key2) and ($LijstWoord='0' or (($parToonSortnaam='1' and PERSONAL/NAME/@surn=$refAchternaam) or ($parToonSortnaam='2' and (PERSONAL/NAME/@sort=$refAchternaam or PERSONAL/NAME/@surn=$refAchternaam))))]">
        <xsl:sort order="ascending" data-type="text" select="PERSONAL/NAME/@surn"/>
        <xsl:sort order="ascending" data-type="text" select="PERSONAL/NAME/@pref"/>
        <xsl:sort order="ascending" data-type="text" select="concat(PERSONAL/NAME/@nick,PERSONAL/NAME/@first)"/>
	    <xsl:sort order="ascending" data-type="text" select="concat(substring(EVENTS/BIRTH/@date,string-length(EVENTS/BIRTH/@date)-3,4),substring(EVENTS/BIRTH/@date,4,2),substring(EVENTS/BIRTH/@date,1,2),substring(EVENTS/BAPT/@date,string-length(EVENTS/BAPT/@date)-3,4),substring(EVENTS/BAPT/@date,4,2),substring(EVENTS/BAPT/@date,1,2))"/>
        <xsl:sort order="ascending" data-type="number" select="PERSONAL/@age"/>
        <xsl:sort order="descending" data-type="text" select="PERSONAL/@of"/>
      	<xsl:variable name="pos"><xsl:value-of select="position()"/></xsl:variable>
        <xsl:call-template name="OverledenWeergave">
          <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
