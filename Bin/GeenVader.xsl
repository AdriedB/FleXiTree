...
// <file>$Workfile: GeenVader.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[not(FATHER) and PERSONAL/NAME/@surn=$refAchternaam])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_zonder_vader"/>
...
<xsl:template name="GeenVaderWeergave">
...
<xsl:template name="KolomWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
       <tr>
        ...
        <xsl:for-each select="PERSOON[not(FATHER) and PERSONAL/NAME/@surn=$refAchternaam]">
          ...
          <xsl:call-template name="GeenVaderWeergave">
            <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
