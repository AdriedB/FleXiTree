...
// <file>$Workfile: MeerOuders.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[count(FATHER)>1 or count(MOTHER)>1])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_meerdan"/><xsl:text> 1 </xsl:text><xsl:value-of select="$Txt_vader"/>/<xsl:value-of select="$Txt_moeder"/>
   	  </xsl:with-param>
...
<xsl:template name="MeerOudersWeergave">
...
<xsl:template name="KolomWeergave">
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
       <tr>
        ...
        <xsl:for-each select="PERSOON[count(FATHER)>1 or count(MOTHER)>1]">
          ...
          <xsl:call-template name="MeerOudersWeergave">
            <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
