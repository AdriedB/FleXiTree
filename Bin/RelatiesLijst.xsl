...
// <file>$Workfile: RelatiesLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <div align="center">
  <h2>
    <xsl:value-of select="$Txt_Overzicht"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_vande_v"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_duur"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_van"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_relaties"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_vande"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_levendepersonen"/>
  </h2>
...
<!--          <xsl:variable name="hoofdId"><xsl:value-of select="concat(../../PERSONAL/NAME/@surn,../../PERSONAL/NAME/@first,../../PERSONAL/NAME/@nick,../../@link)"/></xsl:variable>
          ...
          <xsl:if test="string-length(BEGIN/@date) != 0 and not (END) and //PERSONEN/PERSOON[@link = $relatieLink and not(EVENTS/DEATH)]">
...
<!--  Aldfaer <RELATYPE>_RELATYPE("Huwelijk;Geregistreerd partnerschap;Samenleven;Relatie ;Ontbrekende relatie;Onbekende relatie")</RELATYPE>-->
              ...
              <xsl:if test="$Browser = 'IE'">
                <td align="right">
                    <xsl:if test="string-length(BEGIN/@onz) != 0">
                    <xsl:text>&#177;</xsl:text>
                    </xsl:if>
                    <xsl:value-of select="user:relatieduur(.,string($parTaal),string($Txt_jaar),string($Txt_jaren))"/><xsl:value-of select="$nbsp"/>
                </td>
              </xsl:if>
            </tr>
          </xsl:if>
...
<!--  Aldfaer <RELATYPE>_RELATYPE("Huwelijk;Geregistreerd partnerschap;Samenleven;Relatie ;Ontbrekende relatie;Onbekende relatie")</RELATYPE>-->
              ...
              <xsl:if test="$Browser = 'IE'">
                <td align="right">
                    ...
                    <xsl:value-of select="user:relatieduur(.,string($parTaal),string($Txt_jaar),string($Txt_jaren))"/>
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data">
      <tr>
       ...
       <xsl:call-template name="addColumn">
        ...
        <xsl:if test="$Browser='IE'">
         <xsl:call-template name="addColumn">
            <xsl:with-param name="name"><xsl:value-of select="$Txt_Duur"/></xsl:with-param>
            <xsl:with-param name="sort">34</xsl:with-param>
            <xsl:with-param name="align">left</xsl:with-param>
         </xsl:call-template>
        </xsl:if>
      </tr>
      <xsl:for-each select="PERSOON[not(EVENTS/DEATH)]/RELATIONS/RELA">
