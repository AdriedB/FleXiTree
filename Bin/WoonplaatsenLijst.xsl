...
// <file>$Workfile: WoonplaatsenLijst.xsl $</file><date>$Date: 15-01-15 19:56 $</date>
...
<xsl:template match="PERSONEN">
  ...
  <xsl:call-template name="ShowMenu"/>
   	...
   	<xsl:variable name="cntr">
   	  <xsl:value-of select="count(PERSOON[FACTS/FACT/ADDR/@city=$LijstWoord])"/>
   	</xsl:variable>
  ...
  <div align="center">
   	...
   	<xsl:call-template name="Txt_overzichtshoofdregels">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="$Txt_met"/><xsl:text> </xsl:text><xsl:value-of select="$Txt_woonplaats"/><xsl:text> </xsl:text><xsl:value-of select="$LijstWoord"/>
   	  </xsl:with-param>
...
<xsl:template name="WoonplaatsenlijstWeergave">
  ...
  <xsl:param name="kolom"/>
    ...
    <table border="1" bordercolor="black" cellspacing="0" cellpadding="4" class="tbl-data" width="100%">
         <tr>
          ...
          <xsl:for-each select="PERSOON[FACTS/FACT/ADDR/@city=$LijstWoord]">
       	    <xsl:sort select="sortkey1" order="ascending" data-type="text"/>
     	    <xsl:sort select="sortkey2" order="ascending" data-type="text"/>
            <xsl:sort select="sortkey3" order="ascending" data-type="text"/>
            <xsl:sort select="sortkey4" order="ascending" data-type="text"/>
            <xsl:call-template name="WoonplaatsenlijstWeergave">
              <xsl:with-param name="cntr"><xsl:value-of select="$cntr"/></xsl:with-param>
